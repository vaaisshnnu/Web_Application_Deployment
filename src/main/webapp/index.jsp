<!DOCTYPE html>
<html lang="en">
<head>
  <meta charset="UTF-8" />
  <meta name="viewport" content="width=device-width, initial-scale=1.0"/>
  <title>CI/CD Application Deployment Project with Jenkins | Ganga Vaishnu Reddy</title>
  <style>
    * {
      margin: 0;
      padding: 0;
      box-sizing: border-box;
    }
    body {
      font-family: 'Segoe UI', sans-serif;
      background: linear-gradient(135deg, #1f1c2c, #928dab);
      color: #f0f0f0;
      display: flex;
      flex-direction: column;
      min-height: 100vh;
    }
    header {
      text-align: center;
      padding: 80px 20px 40px;
    }
    header h1 {
      font-size: 2.8rem;
      color: #ffffff;
    }
    header h2 {
      font-size: 1.5rem;
      color: #00ffc8;
      margin-top: 10px;
    }
    p.description {
      font-size: 1.1rem;
      margin-top: 20px;
      max-width: 700px;
      margin-left: auto;
      margin-right: auto;
      line-height: 1.6;
    }
    .buttons {
      margin-top: 30px;
      display: flex;
      justify-content: center;
      gap: 20px;
      flex-wrap: wrap;
    }
    .buttons a {
      text-decoration: none;
      padding: 14px 28px;
      background-color: #00ffc8;
      color: #000;
      border-radius: 30px;
      font-weight: bold;
      transition: background 0.3s;
    }
    .buttons a:hover {
      background-color: #ffffff;
      color: #000;
    }
    .attachments {
      text-align: center;
      margin-top: 40px;
    }
    .attachments a {
      display: inline-block;
      color: #00ffc8;
      text-decoration: underline;
      margin: 5px 10px;
    }
    footer {
      text-align: center;
      padding: 20px;
      font-size: 0.9rem;
      background-color: rgba(0, 0, 0, 0.2);
      margin-top: auto;
    }
  </style>
</head>
<body>

  <header>
    <h1> CI/CD Application Deployment Project with Jenkins</h1>
    <h2>by Ganga Vaishnu Reddy</h2>
    <p class="description">
      This project demonstrates a complete CI/CD pipeline integrating <strong>Jenkins</strong>,
      <strong>GitHub Webhooks</strong>, and deploying the application on an <strong>Apache Tomcat Server</strong>.
      It ensures seamless and automated deployment straight from source to production.
    </p>

<link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.5.0/css/all.min.css">

<div class="buttons">
  <a href="https://github.com/vaaisshnnu/" target="_blank">
    <i class="fab fa-github"></i> GitHub Profile
  </a>
  <a href="https://www.linkedin.com/in/vaaisshnnu-reddy/" target="_blank">
    <i class="fab fa-linkedin"></i> LinkedIn Profile
  </a>
</div>

  <footer>
  👨‍💻 Proudly built using Jenkins, GitHub, Maven, Tomcat, and AWS EC2. <br>
  📈 This is not just a project, it's a proof of practice. <br>
  🔗 <a href="https://github.com/vaaisshnnu" target="_blank" style="color: #00ffc8;">GitHub</a> | <a href="https://www.linkedin.com/in/vaaisshnnu-reddy" target="_blank" style="color: #00ffc8;">LinkedIn</a>
</footer>


</body>
</html>
